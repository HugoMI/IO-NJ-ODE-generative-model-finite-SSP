...
# ------------------------------------------------------------------------------
# Real data to evaluate generative model
IO_BMClass2_real_dict = {
    'model_name': "BMClassification2",
    'nb_paths': 10000,
    'nb_steps': 100,
    'maturity': 1.0,
...
# Specificationd for artificially generating paths with generative scheme
IO_BMClass2_gen_dict = {
    'model_name': 'BMClassification2',
    'nb_gen_paths': 10,
    'nb_steps': 100,
    'delta_t': 0.01,
...
# ------------------------------------------------------------------------------
# --- Brownian Motion Classification 2
IO_BMClass2_models_path = "{}saved_models_BMClassification2/".format(data_path)
param_list_IO_BMClass2 = []
io_nn = ((100, 'relu'), (100, 'relu'))
for ds, dst in [("IO_BMClass2_dict", "IO_BMClass2_dict_test"),]:
    for act in ['relu']:
        _nn = ((size, act),)
        param_dict_IO_BMClass2 = {
            'epochs': [200],
            'batch_size': [200],
            'save_every': [50],
            'learning_rate': [0.001],
            ...
            'dropout_rate': [0.1],
            'ode_nn': [io_nn],
            'readout_nn': [((100, 'relu'), (100, 'relu'), "softmax")],
            'enc_nn': [io_nn],
            'use_rnn': [True],
            ...
            'plot_same_yaxis': [False],
            'use_cond_exp': [True],
            'saved_models_path': [IO_BMClass2_models_path],
            'ode_input_scaling_func': ['identity'],
        }
        param_list_IO_BMClass2 += get_parameter_array(
            param_dict=param_dict_IO_BMClass2)

overview_dict_IO_BMClass2 = dict(
    ids_from=1, ids_to=len(param_list_IO_BMClass2),
    path=IO_BMClass2_models_path,
    params_extract_desc=('data_dict', 'network_size', 'readout_nn',
                         'activation_function_1',
                         'hidden_size', 'batch_size', 'which_loss',
                         'input_sig', 'level', 'coord_wise_tau',
...
plot_paths_IO_BMClass2_dict = {
    'model_ids': [2], 'saved_models_path': IO_BMClass2_models_path,
    'which': 'best',
    'paths_to_plot': [0,1,2,3,4],
    'ylabels': ['$W_t$', '$V_{t, 1}=1_{S_{t}=0}$', '$V_{t, 2}=1_{S_{t}=1}$'],
    'legendlabels': ['true path','I/O NJODE','true conditional expectation','observed'],
    'save_extras': {'bbox_inches': 'tight', 'pad_inches': 0.01},
    'plot_only_evaluate': True,
}
