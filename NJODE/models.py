...
class NJODE(torch.nn.Module):
    ...
    def forward(self, times, time_ptr, X, obs_idx, delta_t, T, start_X,
        ...
        for i, obs_time in enumerate(times):
            ...
            if get_loss:
                ...
                # ********************************************************************************
                loss = loss + LOSS(
                    X_obs=X_obs[:, out_coords], Y_obs=Y[i_obs.long(), :],
        ...
        if return_path:
            ...
            if return_classifier_out:
                ...
                return h, loss, np.array(path_t), torch.stack(path_h), \
                       torch.stack(path_y)[:, :, :], var_path, cl_out
            # return h, loss, np.array(path_t), torch.stack(path_h), \
            #        torch.stack(path_y)[:, :, :dim_to], var_path
            return h, loss, np.array(path_t), torch.stack(path_h), \
                    torch.stack(path_y)[:, :, :], var_path
            # ********************************************************************************
        else:
    ...
    # ********************************************************************************
    def generate_future_path(self, starting_data_pack, steps, delta_t):
        """
        Generate future steps given a sequence of points or one single point as starting step using the trained model.
        :param tarting_data_pack: starting input dictionary
        :param steps: int, number of future steps to generate
        :param delta_t: float, time step size
        :return: torch.tensor, generated path, np.array times
        """
        # The model is set to evaluation mode during generation.
        self.eval()

        # The data concernign the starting point/sequence to generate
        # the future path is retrieved from the starting_data_pack dictionary.

        # This data is necessary to integrate the available past information
        # into the model's hidden state through the forward method.
        starting_dataloader = starting_data_pack['dataloader']
        prior_dt = starting_data_pack['last_dt']
        prior_actual_times = starting_data_pack['last_times'] # Actual times from given data
        
        prior_T = starting_data_pack['last_T']
        model_name = starting_data_pack['model_name']
        starting_data = next(iter(starting_dataloader))

        prior_X = starting_data['X'].to(self.device)
        prior_times = starting_data['times'] # Required to process according to forward
        prior_time_ptr = starting_data['time_ptr']
        prior_obs_idx = starting_data['obs_idx']
        prior_n_obs_ot = starting_data['n_obs_ot']
        prior_M = starting_data['M']
        prior_start_X = starting_data['start_X'].to(self.device)

        # The last hidden state and signature updates are retrieved from
        # the forward method
        last_h, last_sig = self.forward(
            times=prior_times, time_ptr=prior_time_ptr, X=prior_X, obs_idx=prior_obs_idx,
            delta_t=prior_dt, T=prior_T, start_X=prior_start_X, n_obs_ot=prior_n_obs_ot,
            return_path=False, get_loss=False, until_T=False, M=prior_M,
            return_at_last_obs=True,
            start_M=None, which_loss=None)

        batch_size = prior_start_X.size(0)
        
        # The very last observed data point and time are stored to
        # initialize the generation process
        if prior_X.shape[0] == 0:
            # In case of a single starting point, time always start at 0
            last_X = prior_start_X
            last_time = 0.0
        else:
            # In case of a starting sequence, the lat point is taken
            last_X = starting_data['X'][-batch_size:, :].to(self.device)
            last_time = prior_times[-1]

        # The specific input and output coordinates are selected
        last_X_in = last_X[:, self.input_coords]
        last_X_out = last_X[:, self.output_coords]

        times = [last_time]
        
        # Last value of the conditional expectation of the output process
        last_y = self.readout_map(last_h)
        path_y = [last_y]

        tau = torch.zeros((batch_size, 1)).to(self.device)

        # Intial state
        s = last_X_out.clone()
        path_s = [s.detach().cpu().numpy()]
        state = s

        # Initial sojourn (dwell) time
        dwell_time = last_X_in[:, 0:1].clone()
        dwell_times = [dwell_time.detach().cpu().numpy()]


        # Use of neural ODE to evolve 'continouous' in time
        # the hidden state and signature are updated at each step
        for i in range(steps):
            # ODE step
            current_h, current_time = self.ode_step(
                last_h, delta_t, last_time,
                last_X=last_X_in,
                tau=tau,
                signature=last_sig,
                current_y=last_y
            ...
            # Updated value of conditional expectation of the output process
            y = self.readout_map(current_h)
            path_y.append(y)
            # Sampling of the next state with aid of the multinomial distribution
            state_sample = torch.multinomial(y, num_samples=1)
            # One-hot encoding of the sampled state to obtain the indicator vector
            # which corresponds to the output process structure
            s = torch.nn.functional.one_hot(state_sample, num_classes=y.shape[1]).squeeze(1)
            
            new_state = s

            # Identify whether the state has changed or not
            # to update the sojourn (dwell) time accordingly
            new_state_seq = torch.argmax(new_state, dim=1).unsqueeze(1)
            state_seq = torch.argmax(state, dim=1).unsqueeze(1)

            unchanged_states = (new_state_seq == state_seq)*1.
            changed_states = (new_state_seq != state_seq)*1.
            # Update of the sojourn (dwell) times
            new_dwell_time = (dwell_time + unchanged_states * delta_t - changed_states * dwell_time).detach().clone()

            state = new_state
            dwell_time = new_dwell_time

            dwell_times.append(new_dwell_time.detach().cpu().numpy())
            path_s.append(new_state.detach().cpu().numpy())

            # Update of the input data point for the next step
            # depending on the type of stochastic process
            if model_name not in ["RPClassification"]:
                # These proceses only have the indicator of the state
                # as input
                last_X_in = s

                # Calculation of the signature of the path
                # up to the current time. Time and path are concatenated
                # to create the time-augmented path
                times_to_sig = np.reshape(np.array(times), (-1, 1))
                signatures = []
                for b in range(batch_size):
                    path_to_sig = np.array([array[b] for array in path_s])
                    path_data = np.concatenate((times_to_sig, path_to_sig), axis=1)
                    signature = sig.sig(path_data, self.level, 0)
                    signatures.append(signature)
                current_sig = torch.tensor(np.stack(signatures, axis=0), dtype=torch.float32).to(self.device)

            else:
                # For the renewal process it is necessary to add also
                # the updated sojourn (dwell) time as input
                last_X_in[:, 0:1] = dwell_time
                last_X_in[:, 1:3] = state
                
                # Calculation of the signature of the path
                # up to the current time. Time and path are concatenated
                # to create the time-augmented path along with the sojourn (dwell) times
                times_to_sig = np.reshape(np.array(times), (-1, 1))
                signatures = []
                for b in range(batch_size):
                    dwell_times_to_sig = np.array([array[b] for array in dwell_times])
                    path_to_sig = np.array([array[b] for array in path_s])
                    path_data = np.concatenate((times_to_sig, dwell_times_to_sig, path_to_sig), axis=1)
                    signature = sig.sig(path_data, self.level, 0)
                    signatures.append(signature)
                current_sig = torch.tensor(np.stack(signatures, axis=0), dtype=torch.float32).to(self.device)

            # The new generated point is incorporated into the 'memory'
            # of the model through the encoder map
            last_h = self.encoder_map(
                last_X_in,
                mask=None,
                sig=current_sig,
                h=current_h,
            ...
            # Update of the variables for the next iteration
            last_time = current_time
            last_sig = current_sig
            last_y = y
            
        path_S = torch.tensor(np.stack(path_s, axis=1), dtype=torch.float32).to(self.device)
