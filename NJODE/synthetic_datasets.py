...
class RPClassification(StockModel):
    """
    Renewal Process example. It consists of an alternating renewal process with
    two classes. The dwell (sojourn) times in each class are Weibull distributed.
    """

    def __init__(self, nb_paths, nb_steps, S0,
                 maturity, sine_coeff=None, **kwargs):
        super().__init__(
            drift=None, volatility=None, nb_paths=nb_paths,
            nb_steps=nb_steps, S0=S0, maturity=maturity,
        ...
        self.loss = None
        # print("len(S0)", len(S0))
        assert len(S0) == 3, "S0 (start data) should be of length 3: [dwell time, 0 or 1, 0 or 1]"
        assert S0[1] in [0., 1.] and S0[2] in [0., 1.], "S0 can only take value 0 or 1, but not both the same."
        self.nb_classes = 2

        # Paramters for the Weibull distributions for each dwell (sojourn) time
        # for each state
        self.shape0 = 1.5
        self.scale0 = 0.1
        self.shape1 = 2.0
    ...
    def curr_cond_exp(self, y, delta_t, current_t, last_obs, duration_t, last_obs_time):
        """
        Args:
            last_obs, duration_t, last_obs_time : np.ndarray with shape (n,)
            current_t, delta_t : scalars
            N : total number of Monte Carlo simulations per row
            chunk_size : number of paths to simulate at once (memory control)
            device : "cpu" or "cuda"
        """
        N=100000
        chunk_size=10000
        ...
        def sample_residual_weibull(shape, scale, a, size, device=device):
            """Direct residual Weibull sampling (inverse transform)."""
            a = torch.tensor(a, dtype=torch.float32, device=device) # current age
            scale = torch.tensor(scale, dtype=torch.float32, device=device)

            Sa = torch.exp(- (a / scale) ** shape)  # survival at a
            U = torch.rand(size, device=device, dtype=torch.float32)
            F = 1 - Sa * (1 - U)
            T = scale * torch.pow(-torch.log(1 - F), 1.0 / shape)
            return T - a # residual time

        def sample_weibull(shape, scale, size, device=device):
            """Standard Weibull sampling (inverse transform)."""
            scale = torch.tensor(scale, dtype=torch.float32, device=device)
            U = torch.rand(size, device=device, dtype=torch.float32) # uniform random variable
            return scale * torch.pow(-torch.log(1 - U), 1.0 / shape) # inverse transform

        # Convert arrays to tensors
        last_obs = torch.as_tensor(last_obs, dtype=torch.int64, device=device)
        duration_t = torch.as_tensor(duration_t, dtype=torch.float32, device=device)
        last_obs_time = torch.as_tensor(last_obs_time, dtype=torch.float32, device=device)
        current_t_ = current_t + delta_t

        n = last_obs.shape[0]
        shape0, scale0 = self.shape0, self.scale0
        shape1, scale1 = self.shape1, self.scale1
        ...
        for chunk in range(n_chunks):
            m = min(chunk_size, N - chunk * chunk_size)  # paths in this chunk

            # Initial residual times
            times = torch.empty((n, m), dtype=torch.float32, device=device)
            states = torch.empty((n, m), dtype=torch.int8, device=device)

            for k in range(n):
                i = int(last_obs[k].item())
                a = float(duration_t[k].item())
                # s = float(last_obs_time[k].item() - delta_t)
                s = float(last_obs_time[k].item())

                if i == 0:
                    resid = sample_residual_weibull(shape0, scale0, a, m, device)
                else:
                    resid = sample_residual_weibull(shape1, scale1, a, m, device)

                times[k] = s + resid
                states[k] = 1 - i

            # Simulate until current_t
            active = times < current_t_
            while torch.any(active):
                idx_rows, idx_cols = active.nonzero(as_tuple=True)
                active_states = states[idx_rows, idx_cols]

                sojourns = torch.empty_like(active_states, dtype=torch.float32, device=device)
                mask0 = active_states == 0
                mask1 = ~mask0

                if mask0.any():
                    sojourns[mask0] = sample_weibull(shape0, scale0, mask0.sum().item(), device)
                if mask1.any():
                    sojourns[mask1] = sample_weibull(shape1, scale1, mask1.sum().item(), device)

                times[idx_rows, idx_cols] += sojourns
                states[idx_rows, idx_cols] = 1 - states[idx_rows, idx_cols]
                active = times < current_t_

            # Final states (just before crossing current_t)
            # so the final state results by flipping the last state
            final_state = 1 - states
            probs += (final_state == 1).float().mean(dim=1)

        # Average across chunks
        probs /= n_chunks
        probs0 = 1 - probs
        probs_full = torch.stack((probs0, probs), dim=1)


        return probs_full.cpu().numpy()
    ...
    def compute_cond_exp(self, times, time_ptr, X, obs_idx, delta_t, T, start_X,
        ...
        # Batch size
        bs = start_X.shape[0]
        duration_t = start_X[:, 0]
        # This selects the observed values for state 1 (the one of interest)
        last_obs = start_X[:, 2]
        last_obs_time = np.zeros(bs)

        # Initialization of conditional expectation values
        y = np.zeros((bs, self.nb_classes))
        # for state 0 and state 1, respectively
        y[:, 1] = (start_X[:, 0] > 0)*1.
        y[:, 0] = (y[:, 1] == 0.0)*1.
        ...
        for i, obs_time in enumerate(times):
            ...
            while current_time < (obs_time - 1e-10 * delta_t):
                ...
                y = self.curr_cond_exp(
                    ...
                    duration_t=duration_t, last_obs_time=last_obs_time)
                current_time = current_time + delta_t_
            ...
            Y_bj = y
            temp = copy.deepcopy(y)
            duration_t[i_obs] = X_obs[:, 0]
            last_obs[i_obs] = X_obs[:, 2]
            ...
            if get_loss:
                loss = loss + compute_loss(
                    X_obs=X_obs[:, 1:], # both states must be considered
                    Y_obs=Y[i_obs],
                    Y_obs_bj=Y_bj[i_obs],
                    n_obs_ot=n_obs_ot[i_obs], batch_size=batch_size,
                    weight=weight, M_obs=M_obs[:, 1:],
                    which_loss=which_loss)
            if return_path:
                path_t.append(obs_time)
                path_y.append(y)
        ...
        while current_time < T - 1e-10 * delta_t:
            if current_time < T - delta_t:
                ...
                delta_t_ = T - current_time
            y = self.curr_cond_exp(
                y, delta_t_, current_time, last_obs,
                duration_t, last_obs_time)
            current_time = current_time + delta_t_
    ...
    def generate_paths(self, start_X=None):
        ...
        # State-specific dwell time distributions
        def sample_dwell_time_state0():
            return scipy.stats.weibull_min.rvs(c=self.shape0, scale=self.scale0)

        def sample_dwell_time_state1():
            return scipy.stats.weibull_min.rvs(c=self.shape1, scale=self.scale1)
        
        # Simulation
        def simulate_discrete_renewal(T, dt=0.1):
            start_state = 0
            current_state = start_state
            dwell_times = []
            states = []

            # Loop to generate dwell times until time T
            while sum(dwell_times) <= T :
                dwell = sample_dwell_time_state0() if current_state == 0 else sample_dwell_time_state1()
                dwell_times.append(dwell)
                states.append(current_state)
                current_state = 1 - current_state

            # Dwell times are accumulated
            dwell_times = np.array(dwell_times)
            dwell_times = np.cumsum(dwell_times)

            path = np.zeros(self.nb_steps + 1)
            path[0] = start_state
            # This the age time, the time spent in the current state
            spent_times = np.zeros(self.nb_steps + 1)
            spent_times[0] = 0.0

            # It computes the state and the age time at each discrete time step
            for k in range(1, self.nb_steps + 1):
                sup_idx = np.argmin(dwell_times <= k*dt)
                inf_idx = sup_idx - 1
                path[k] = states[sup_idx]
                spent_times[k] = k*dt - dwell_times[inf_idx] if inf_idx >= 0 else k*dt
        
            return path, spent_times

        for i in range(self.nb_paths):
            states, spent_times = simulate_discrete_renewal(self.maturity, dt)
            spot_paths[i, 0, :] = spent_times
            spot_paths[i, 1, :] = (states == 0.0)*1.
            spot_paths[i, 2, :] = (states == 1.0)*1.
...
class BMClassification2(BMClassification):
    """
    A Brownian Motion classification example. S_t = 1_{W_t >= 0} where
    W is a standard Brownian motion.
    X is the input data composed by the indicator vector of the states
    and y is the conditional expectation.
    """

    def __init__(self, nb_paths, nb_steps, maturity, alpha, dimension, n_particles=100000,
                 **kwargs):
        super(BM2DCorr, self).__init__(
            drift=None, volatility=None, nb_paths=nb_paths,
            nb_steps=nb_steps, S0=None, maturity=maturity,
            sine_coeff=None
        )
        self.alpha = alpha
        assert dimension == 2, "dimension has to be set to 1 for this dataset"
        self.path_t = None
        self.loss = None
        self.nb_classes = dimension
        self.n_particles = n_particles
    
    class SignParticleFilterBatch:
        """
        Batch particle filter for sign observations of Brownian motion.

        - particles shape: (batch, n_particles)
        - t_last shape: (batch,)
        - update accepts t_new, sign_new of shape (batch,)
        - predict_prob(t_future) returns tensor (batch,) with estimated probabilities.

        Chunked version: updates and predictions are processed in blocks of size `chunk_size`
        to allow very large n_particles without running out of memory.
        """

        def __init__(self, batch, n_particles=5000, device="cpu", dtype=torch.float32, seed=None, chunk_size=10000):
            ...
            # particles: W_{t_last} samples for each batch element
            self.particles = torch.zeros((self.batch, self.n_particles),
                                        dtype=self.dtype, device=self.device)
            # last observation time for each batch element
            self.t_last = torch.zeros(self.batch, dtype=self.dtype, device=self.device)

            # standard normal for cdf/icdf
            self.std_normal = torch.distributions.Normal(loc=0.0, scale=1.0)

            # numerical epsilon to avoid exact 0/1 in inv-cdf
            self._eps = 1e-12

        def update(self, t_new, sign_new):
            """
            Update the filter with new-observation times and signs.

            Args:
                t_new: tensor-like of shape (batch,) or broadcastable to (batch,)
                sign_new: tensor-like of shape (batch,) with values +1 or -1
            """
            t_new = torch.as_tensor(t_new, dtype=self.dtype, device=self.device).reshape(-1)
            sign_new = torch.as_tensor(sign_new, device=self.device).reshape(-1)

            if t_new.shape[0] != self.batch or sign_new.shape[0] != self.batch:
                raise ValueError("t_new and sign_new must have shape (batch,) matching filter.batch")

            dt = t_new - self.t_last
            if torch.any(dt < 0):
                raise ValueError("t_new must be >= last time for each batch element")

            for b in range(self.batch):
                if dt[b] == 0:
                    # it enforces sign on existing particles, with less weight
                    # on those violating the sign
                    s = int(sign_new[b].item())
                    mask = (s * self.particles[b, :] < 0)
                    if mask.any():
                        tiny = 1e-6
                        self.particles[b, mask] = tiny * s

                elif dt[b] > 0:
                    # Parameters of the truncated normal
                    mean = self.particles[b, :]
                    std = torch.sqrt(dt[b])
                    sgn = sign_new[b].item()

                    # Splitting the particles update into chunks to save memory
                    for start in range(0, self.n_particles, self.chunk_size):
                        end = min(start + self.chunk_size, self.n_particles)
                        m_chunk = mean[start:end]

                        # Truncation boundary
                        a = (-m_chunk) / std
                        cdf_a = self.std_normal.cdf(a)
                        cdf_a = torch.clamp(cdf_a, min=self._eps, max=1.0 - self._eps)

                        u = torch.rand_like(cdf_a, device=self.device, dtype=self.dtype)
                        # Sample from truncated right tail
                        if sgn == 1:
                            u = cdf_a + (1.0 - cdf_a) * u
                        # Sample from truncated left tail
                        else:
                            u = cdf_a * u

                        # Clamped uniform samples
                        u = torch.clamp(u, min=self._eps, max=1.0 - self._eps)
                        # Inverse CDF transform
                        z = self.std_normal.icdf(u)

                        # Truncated normal samples
                        new_particles = m_chunk + std * z
                        new_particles = torch.nan_to_num(new_particles, nan=0.0, posinf=1e6, neginf=-1e6)

                        self.particles[b, start:end] = new_particles

            self.t_last = t_new.clone()

        def predict_prob(self, t_future):
            """
            Compute P(X_{t_future} = 1 | current observations) for each batch element.
            Args:
                t_future: scalar or tensor shape (batch,) (broadcastable)
            Returns:
                tensor of shape (batch, 2) with probabilities (P(sign=-1), P(sign=+1))
            """
            t_future = torch.as_tensor(t_future, dtype=self.dtype, device=self.device).reshape(-1)
            if t_future.numel() == 1:
                t_future = t_future.expand(self.batch)
            if t_future.shape[0] != self.batch:
                raise ValueError("t_future must be scalar or shape (batch,)")

            dt_future = t_future - self.t_last
            if torch.any(dt_future < 0):
                raise ValueError("t_future must be >= last observed time for each batch element")

            # Initializacion of probabilities values for state 1
            probs = torch.empty(self.batch, dtype=self.dtype, device=self.device)

            for b in range(self.batch):
                n_parts = self.n_particles
                dt = dt_future[b].item()
                # The probability is estimated as a proportion of the available particles
                if dt == 0:
                    total = 0.0
                    for start in range(0, n_parts, self.chunk_size):
                        end = min(start + self.chunk_size, n_parts)
                        total += (self.particles[b, start:end] >= 0).to(self.dtype).sum()
                    probs[b] = total / n_parts
                # The probability is computed via the CDF of the normal distribution
                else:
                    std_future = torch.sqrt(torch.tensor(dt, dtype=self.dtype, device=self.device))
                    total = 0.0
                    for start in range(0, n_parts, self.chunk_size):
                        end = min(start + self.chunk_size, n_parts)
                        vals = self.particles[b, start:end] / std_future
                        vals = torch.clamp(vals, min=-1e10, max=1e10)
                        total += self.std_normal.cdf(vals).sum()
                    probs[b] = total / n_parts

            # Probabilities of state 0
            probs0 = 1 - probs
            probs_full = torch.stack((probs0, probs), dim=1)
            return probs_full

    def curr_cond_exp(self, filt, delta_t, current_t, last_obs, last_obs_time):
        last_sign_obs = 2*last_obs - 1  # convert to -1 or +1 each observed state
        filt.update(last_obs_time, last_sign_obs)
        return filt.predict_prob(current_t + delta_t).detach().cpu().numpy()
    ...
    def compute_cond_exp(self, times, time_ptr, X, obs_idx, delta_t, T, start_X,
        ...
        y = np.zeros((bs, self.nb_classes))
        # for state 0 and state 1, respectively
        y[:, 0] = (start_X[:, 0] <= self.alpha)*0.5
        y[:, 1] = (start_X[:, 0] >= self.alpha)*0.5
        ...
        # Initializing (instance of) the particle filter
        filt = self.SignParticleFilterBatch(batch=bs, n_particles=self.n_particles, device=device)
        ...
        for i, obs_time in enumerate(times):
            ...
            while current_time < (obs_time - 1e-10 * delta_t):
                if current_time < obs_time - delta_t:
                    ...
                    delta_t_ = obs_time - current_time
                y = self.curr_cond_exp(
                    filt, delta_t_, current_time, last_obs=last_obs,
                    last_obs_time=last_obs_time)
                current_time = current_time + delta_t_
        ...
        while current_time < T - 1e-10 * delta_t:
            if current_time < T - delta_t:
                ...
                delta_t_ = T - current_time
            y = self.curr_cond_exp(
                filt, delta_t_, current_time, last_obs, last_obs_time)
            current_time = current_time + delta_t_
    ...
    def generate_paths(self):
        spot_paths = np.zeros(
            (self.nb_paths, 3, self.nb_steps + 1))
        dt = self.dt
        ...
        spot_paths[:, 0, 1:] = W[:, :]
        spot_paths[:, 2, :] = (spot_paths[:, 0, :] >= self.alpha)*1.
        spot_paths[:, 1, :] = (spot_paths[:, 0, :] < self.alpha)*1.

        # stock_path dimension: [nb_paths, dimension + 1, time_steps]
        # the extra dimension is just to stote the standard Brownian motion
        return spot_paths, dt
...
DATASETS = {
    ...
    "BMClassification2": BMClassification2,
